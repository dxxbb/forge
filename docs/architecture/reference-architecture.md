...
- `Capture / Store / Projections` 是结构块
- 它们不是一条单纯的时间线
- 真正的数据流要分成 `写入流`、`使用流`、`回流`

最小理解方式：

1. `写入流`：
   新信息先到 `Capture`，整理后进 `Store`，再生成 `Projections`
2. `使用流`：
   平台平时主要读自己的入口文件，不默认直读 `Store`
3. `回流`：
   平台侧确认的新规则先回 `Capture`，再并进 `Store`，再重生 `Projections`
...
1. 平台先读自己的正式入口
2. Phase 1 默认先靠 projection 工作
3. 本地平台需要更多内容时，再显式打开相关文件或走 MCP

这里要故意写死一条边界：

- Phase 1 不假设平台会自动直读本地 `Store`
...
1. 平台侧先产生新规则或新结论
2. 先回写 `Capture`
3. 需要当天生效时，先补目标平台 projection
4. 晚些时候再 `Consolidate` 进 `Store`
5. 再统一 `Project`
